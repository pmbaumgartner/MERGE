...
class Lexemes(object):
    ...
    def set_merge_token(self, merge_token):
        ...
        for wordstr, position in self.merge_token.lex:
            if position > 0:
                self.curr_satellite_lex = NTlexeme(
                    lex=self.merge_token.lex, token_index=position
                )
    ...
    def add_merge_token(self, turn_number, satellite_position, merge_token_leftanchor):
        ...
        self.absolute_position_in_turn = (
            self.satellite_position + self.merge_token_leftanchor
        )
        self.loc = self._locations_to_locations[
            (self.turn_number, self.satellite_position)
        ]
        self._lexemes_to_locations[
            self.satellite_lexemes[
                self.satellite_position - self.merge_token_leftanchor
            ]
        ].add(self.loc)
        self._locations_to_lexemes[self.turn_number][
    ...
    def count_frequencies(self):
        ...
        for key, value in self._lexemes_to_locations.items():
    ...
    def get_turn_lengths(self):
        ...
        for turnindex, turnlength in self.turn_lengths.items():
    ...
    def get_lexeme(self, turnindex, wordindex):
        ...
        self.leftanchor_lexeme = self._locations_to_lexemes[self.turnindex][
            self.leftanchor_pos_in_turn
        ]
...
class InitCorpus(object):
    ...
    def initialize(
        ...
        self.corpus_files = [
            fn for fn in os.listdir(self.corpus_dir_path) if fn.lower().endswith(ext)
        ]
        self.lineindex = 0
        ...
        for line in self._get_line_from_files():
            ...
            for wordindex, word in enumerate(self._tokenize_line(self.line)):
                ...
                if self.already_seen_words.is_present(self.word):
                    ...
                    self.all_lexemes.add_location(
                        self.curr_lexeme, self.lineindex, self.wordindex
                    )
                else:
                    self.new_lexeme = self._create_lexeme(self.word)
                    self.already_seen_words.add_word_with_lexeme(
                        self.word, self.new_lexeme
                    )
                    self.all_lexemes.add_location(
                        self.new_lexeme, self.lineindex, self.wordindex
                    )
    ...
    def _create_lexeme(self, wordstring):
        ...
        new_lexeme = NTlexeme(
            lex=(NTword(wordstr=wordstring, position=0),), token_index=0
        )
