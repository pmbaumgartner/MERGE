...
class LogLikelihoodManager(object):
    ...
    def calc_ll_for_single_table(self):
        ...
        self.expB = (self.corpus_size-self.table["El2 Freq"]) / self.corpus_size*self.table["El1 Freq"]
        self.expC = self.table["El2 Freq"]/self.corpus_size * (self.corpus_size-self.table["El1 Freq"])
        self.expD = (self.corpus_size-self.table["El2 Freq"]) / self.corpus_size*(self.corpus_size-self.table["El1 Freq"])
            
        self.llA = self.obsA*np.log(self.obsA/self.expA)
            
        self.real_vals = np.where(self.obsB!=0)[0]
        self.llB = np.zeros(len(self.table))
        self.llB[self.real_vals] = self.obsB[self.real_vals]*np.log(self.obsB[self.real_vals]/self.expB[self.real_vals])
            
        self.real_vals = np.where(self.obsC!=0)[0]
        self.llC = np.zeros(len(self.table))
        self.llC[self.real_vals] = self.obsC[self.real_vals]*np.log(self.obsC[self.real_vals]/self.expC[self.real_vals])
    ...
    def get_winner(self):
        ...
        for tablenum,ll_series in self.ll_indexer.iteritems():
            self.tablenum = tablenum
            self.ll_series = ll_series
            self.winner = self.ll_series.idxmax()
            self.winner_ll = self.ll_series[self.winner]
            self.winner_triple = NTwinner(winner=self.winner,winner_ll=self.winner_ll,winner_tablenum=self.tablenum)
