...
NTcontextpos = namedtuple(
    "NTcontextpos", "context_pos satellite_pos gapsize merge_token_leftanchor"
)
...
class BigramUpdater(object):
    def get_new_and_conflicting_bigrams(
        self, all_bigrams, all_lexemes, merge_token, winner_info, max_gapsize
    ):
        ...
        # Class instances
        self.context_pos_manager = ContextPositionManager(
            self.all_lexemes, self.max_gapsize
        )
        self.vicinity_lex_manager = VicinityLexemeManager(
            self.all_lexemes, self.merge_token
        )

        # Program
        self.winner_locations = self.all_bigrams.bigrams_to_locations[
        ...
        for turn_number, merge_token_leftanchor in self.winner_locations:
            ...
            if not self.bigram_is_among_existing_conflicting_bigrams():
                ...
                self.context_positions = (
                    self.context_pos_manager.generate_positions_around_satellites(
                        self.turn_number,
                        self.satellite_positions,
                        self.merge_token_leftanchor,
                    )
                )
                self.vicinity_lex_manager.create_bigrams_with_lexemes_surrounding_satellites(
                    self.turn_number,
                    self.context_positions,
                    self.satellite_positions,
    ...
    def update_all_lexemes_with_merge_tokens(self):
        ...
        for (
            turn_number,
            satellite_position,
        ), merge_token_leftanchor in self.merged_satellite_positions.items():
            self.turn_number = turn_number
            self.satellite_position = satellite_position
            self.merge_token_leftanchor = merge_token_leftanchor
            self.all_lexemes.add_merge_token(
                self.turn_number, self.satellite_position, self.merge_token_leftanchor
            )
    ...
    def bigram_is_among_existing_conflicting_bigrams(self):
        ...
        if (
            self.turn_number,
            self.merge_token_leftanchor,
        ) in self.vicinity_lex_manager.conflicting_bigrams.bigrams_to_locations[
            self.winner_info.winner
        ]:
            return True
        else:
    ...
    def get_satellite_positions(self):
        ...
        self.satellite_positions = [
            satellite_position + self.merge_token_leftanchor
            for wordstr, satellite_position in self.merge_token.lex
        ]
    ...
    def store_satellite_positions(self):
        ...
        for satellite_position in self.satellite_positions:
            self.satellite_position = satellite_position
            self.merged_satellite_positions[
                (self.turn_number, self.satellite_position)
            ] = self.merge_token_leftanchor
...
class ContextPositionManager(object):
    ...
    def generate_positions_around_satellites(
        self, turn_number, satellite_positions, merge_token_leftanchor
    ):
    ...
    def get_left_context_position(self):
        ...
        if self.left_context_position >= 0:
            self.curr_contextpos = NTcontextpos(
                context_pos=self.left_context_position,
    ...
    def get_right_context_position(self):
        ...
        if self.right_context_position < self.curr_turn_length:
            self.curr_contextpos = NTcontextpos(
                context_pos=self.right_context_position,
                satellite_pos=self.satellite_position,
                gapsize=self.curr_gapsize,
                merge_token_leftanchor=self.merge_token_leftanchor,
            )
...
class VicinityLexemeManager(object):
    ...
    def create_bigrams_with_lexemes_surrounding_satellites(
        self,
        turn_number,
        context_positions,
        satellite_positions,
        ...
        for context_position_info in self.context_positions:
            ...
            if self.context_position not in self.satellite_positions:  # the context
                (
                    self.premerge_lexeme,
                    self.premerge_leftanchor,
                ) = self.all_lexemes.get_lexeme(
                    self.turn_number, self.satellite_position
                )
                if (
                    self.turn_number,
                    self.context_position,
                ) in self.merged_satellite_positions:
                    self.context_lexeme = self.merge_token
                    self.context_leftanchor = self.merged_satellite_positions[
                        (self.turn_number, self.context_position)
                    ]
    ...
    def create_new_bigram(self):
        ...
        if self.context_lexeme_anchor_to_the_left_of_target_lexeme_anchor(
            target=self.merge_token_leftanchor
        ):
            self.make_location_tuple(leftanchor=self.context_leftanchor)
            self.gap_between_leftanchors = (
                self.merge_token_leftanchor - self.context_leftanchor - 1
            )
            self.new_bigrams.save_bigram_data(
                self.context_lexeme,
                self.merge_token,
                self.gap_between_leftanchors,
                self.location_tuple,
            ...
            self.gap_between_leftanchors = (
                self.context_leftanchor - self.merge_token_leftanchor - 1
            )
            self.new_bigrams.save_bigram_data(
                self.merge_token,
                self.context_lexeme,
    ...
    def create_conflicting_bigram(self):
        ...
        if self.context_lexeme_anchor_to_the_left_of_target_lexeme_anchor(
            target=self.premerge_leftanchor
        ):
            self.make_location_tuple(leftanchor=self.context_leftanchor)
            self.gap_between_leftanchors = (
                self.premerge_leftanchor - self.context_leftanchor - 1
            )
            self.conflicting_bigrams.save_bigram_data(
                self.context_lexeme,
                self.premerge_lexeme,
                self.gap_between_leftanchors,
                self.location_tuple,
            ...
            self.gap_between_leftanchors = (
                self.context_leftanchor - self.premerge_leftanchor - 1
            )
            self.conflicting_bigrams.save_bigram_data(
                self.premerge_lexeme,
                self.context_lexeme,
    ...
    def make_location_tuple(self, leftanchor=None):
        ...
        self.location_tuple = self.all_lexemes.get_extant_loc_object(
            (self.turn_number, leftanchor)
        )
