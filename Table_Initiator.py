...
class Tables(object):
    ...
    def add_table(self, columnlists):
        ...
        self.bgrs_to_tablenum = {
            bgr: self.tablenum for bgr in self.columnlists.row_indices
        }  # separate function to update bigrams_to_table?
        self.bigrams_to_table.update(self.bgrs_to_tablenum)

    def create_df(self):

        self.data = {
            "Bigram Freq": self.columnlists.bgr_freqs,
    ...
    def update_cell_value(self, bigram=None, column=None, new_value=None):
        ...
        self.all_tables[self.bigrams_to_table[bigram]].at[bigram, column] = new_value

    def cleanup(self):

        self.updated_tables = {}
        for tablenum, table in self.all_tables.items():
    ...
    def add(self, columnlists, type_count):
        ...
        for tablenum, size in self.table_sizes.items():
        ...
        if self.available_tables:
            ...
            self.bgrs_to_tablenum = {
                bgr: self.curr_table_num for bgr in self.columnlists.row_indices
            }
            self.bigrams_to_table.update(self.bgrs_to_tablenum)
...
class InitCandidateTable(object):
    def initialize(self, all_bigrams, all_lexemes, max_row_count=50000):
        ...
        for index, bgr, freq in self.all_bigrams.iter_bgrs_and_freqs():
            ...
            if (
                index + 1
            ) % self.max_row_count == 0 or index == self.all_bigrams.type_count - 1:
