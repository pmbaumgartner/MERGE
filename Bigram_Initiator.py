...
class Bigrams(object):
    ...
    def create_bigram(self,left_lex,right_lex,gapsize):
        ...
        return NTbigram(el1=left_lex, el2=right_lex,
                        gapsize=gapsize)
    
    def iter_bgrs_and_freqs(self):
        
        for index,(bgr,freq) in enumerate(self.bigrams_to_freqs.iteritems()):
    ...
    def get_bigrams(self):
        ...
        for bigram in self.bigrams_to_freqs.iterkeys():
            yield bigram
    ...
    def deduct_freqs(self, other_bigrams_instance):
        ...
        for index,bigram,freq in self.other_bigrams_instance.iter_bgrs_and_freqs():
            self.bigram = bigram
            self.freq = freq
            self.locations = self.other_bigrams_instance.bigrams_to_locations[self.bigram]
            if self.bigram not in self.bigrams_to_freqs:
                raise Exception("bigram not in bigrams_to_freqs")
            self.bigrams_to_freqs[self.bigram] -= self.freq             #IF BIGRAM IS NOT IN COUNTER, THIS SYNTAX WILL STICK IT BACK IN!!!!!!!
            ...
            if self.bigrams_to_freqs[self.bigram] < 1:
                ...
                self.left_lex_to_bigrams[(self.bigram.el1,self.bigram.gapsize)].remove(self.bigram)
                self.right_lex_to_bigrams[(self.bigram.el2,self.bigram.gapsize)].remove(self.bigram)
    ...
    def add(self, other_bigrams_instance):
        ...
        for index,bigram,freq in self.other_bigrams_instance.iter_bgrs_and_freqs():
            ...
            self.bigrams_to_locations[self.bigram] = self.curr_locs.union(self.other_bigrams_instance.bigrams_to_locations[self.bigram])
            
        for (el1,gapsize),bigrams in self.other_bigrams_instance.left_lex_to_bigrams.iteritems():
            self.el1 = el1
            self.gapsize = gapsize
            self.bigrams = bigrams
            self.curr_left_lex_to_bigrams = self.left_lex_to_bigrams[(self.el1,self.gapsize)]
            self.left_lex_to_bigrams[(self.el1,self.gapsize)] = self.curr_left_lex_to_bigrams.union(self.bigrams)
            
        for (el2,gapsize),bigrams in self.other_bigrams_instance.right_lex_to_bigrams.iteritems():
            self.el2 = el2
            self.gapsize = gapsize
            self.bigrams = bigrams
            self.curr_right_lex_to_bigrams = self.right_lex_to_bigrams[(self.el2,self.gapsize)]
            self.right_lex_to_bigrams[(self.el2,self.gapsize)] = self.curr_right_lex_to_bigrams.union(self.bigrams)
...
class InitBigramData(object):
    ...
    def initialize(self, lexeme_data, max_gapsize):
        ...
        for turnindex,turnlength,curr_gapsize in self._turns_X_gapsizes():
            ...
            for left,right,location in self._get_elmnts_and_loc():
                self.bigrams.save_bigram_data(left,
                                              right,
                                              self.curr_gapsize,
                                              location)
    ...
    def _get_elmnts_and_loc(self):
        ...
        for lexindex in range(self.rightmost_leftedge):
            ...
            self._left,self.left_anchor_pos = self.lexeme_data.get_lexeme(self.turnindex,
                                                self.lexindex)
            self._right,self.right_anchor_pos = self.lexeme_data.get_lexeme(self.turnindex,
                                                 self.lexindex +
                                                 self.curr_gapsize
                                                 + 1)
            self._location = self.lexeme_data.get_extant_loc_object((self.turnindex,
                                                                self.lexindex))
